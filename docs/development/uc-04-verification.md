...
| --- | --- | --- | --- |
| FR-WF-01 / BR-01 | `WorkflowDesignService` requires exactly one typed Execute step before identity generation or persistence. The editor exposes standard-step add/remove/move actions and a workflow-level live error. | Domain, application, controller, widget, Drift, and integration tests | Missing or duplicated Execute rejects save with `workflow.execute.count`; the repository and saved revision remain unchanged. |
...
| FR-WF-07 | Structural validation checks name, work-item approach, at least one ordered non-empty step, and exactly one Execute before the atomic repository call. | Application validation tests; indexed editor error tests; repository rollback and migration tests | AF-01 and AF-02 do not alter the stored aggregate or revision; storage failures are sanitized and stale edits are typed. Agent readiness is deliberately separate and arrives in UC-05. |
| FR-WF-08 / BR-06 | `WorkItemType` supports use case, GitHub issue, and free-form task, with a required editor selection and durable typed value. | Domain/application and editor tests; Drift round-trip tests | All three approaches validate, save, reopen, and remain available to UC-06 run materialization. |
...
| AF-01: Execute missing or duplicated | Application, controller, widget, and repository tests reject the invariant without save, ID consumption, or revision change and identify the workflow-level remediation. |
