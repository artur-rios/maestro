...
- Local full-suite result: 741 tests passed.
...
| FR-TE-03 | `PtyTerminalPort` validates the folder before spawning and sends it as the PTY working directory. | Port tests reject a missing folder without launch and assert the launch request uses the project directory; integration coverage starts a real shell in a temporary project folder. |
...
| AF-02 — project folder unavailable | Both application and platform guards refuse startup before spawning, leave the project record untouched, and the workspace omits the terminal for unavailable folders. |
