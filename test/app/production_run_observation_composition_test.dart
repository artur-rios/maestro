...
void main() {
  ...
  testWidgets(
    'GivenProductionProjectTools_WhenViewportChanges_ThenHistoryAndUpdatesShareResponsiveGeometry',
    (tester) async {
      final root = await Directory.systemTemp.createTemp(
        'maestro-project-tools-geometry-',
      );
      addTearDown(() => root.delete(recursive: true));
      final database = MaestroDatabase(NativeDatabase.memory());
      final composition = await app.composeProductionApp(
          ...
          MaterialApp(
            home: Scaffold(
              body: Align(
                ...
                child: SizedBox(
                  width: width,
                  child: composition.historyBuilder(
                    _BuildContextStub(),
                    'actor-1',
                    _project(root.path),
                  ),
      ...
      Finder cardContaining(String text) =>
          find.ancestor(of: find.text(text), matching: find.byType(Card));

      await pumpProjectTools(900);
      final desktopHistory = cardContaining('History and audit');
      final desktopUpdates = cardContaining('Application updates');
      expect(tester.getSize(desktopHistory).width, 640);
      expect(tester.getSize(desktopUpdates).width, 640);
      expect(
        tester.getTopLeft(desktopUpdates).dx,
        tester.getTopLeft(desktopHistory).dx,
      );

      await pumpProjectTools(500);
      final narrowHistory = cardContaining('History and audit');
      final narrowUpdates = cardContaining('Application updates');
      expect(tester.getSize(narrowHistory).width, 500);
      expect(tester.getSize(narrowUpdates).width, 500);
      expect(
        tester.getTopLeft(narrowUpdates).dx,
        tester.getTopLeft(narrowHistory).dx,
      );
